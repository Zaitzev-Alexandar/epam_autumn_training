...
namespace FiguresBoxLib
{
    ...
    public class FiguresBox
    {
        ...
        public void AddFigure(IFigure figure)
        {
            ...
            for (int i = 0; i < Length; i++)
            {
                if (figuresBox[i] != null && figuresBox[i].Equals(figure))
                    throw new Exception("You can't add equal figures");
        ...
        /// <summary>
        /// Extract all plastic figures without coloring.
        /// </summary>
        /// <returns></returns>
        public IFigure[] GetAllPlasticFiguresWithoutColoring()
        {
            int length = 0;
            for (int i = 0; i < Length; i++)
            {
                if (figuresBox[i] != null && figuresBox[i].GetType().BaseType == typeof(PlasticSheet) && ((Sheet)figuresBox[i]).IsColored == false)
                    length++;
            }
        ...
        public override string ToString()
        {
            ...
            stringBuilder.Append("Figures in box:\n");
            for (int i = 0; i < Length; i++)
            {
                if (figuresBox[i] != null)
                {
